...
#include "packets/bnet/BNetAuthLogon.h"

namespace Plugins
{
	namespace BNet
	{
		bool AuthLogon::Process( Network::TcpClient& cl, Utils::Stream& in )
		{
			...
			Packets::BNet::BNetAuthLogon packet;
			LOG_TRACE(("%s is logging in...", username.c_str()));
			Cache::UserCacheItem::Pointer ptr = Cache::userCache[username.c_str()];
			if(ptr.get() == NULL || !ptr->IsExist()/* || Ban.share.checkExpire(user)*/)
			{
				packet.status = 1;
				memset(packet.salt, 0, 32);
				memset(packet.pubkey, 0, 32);
			}
			else if((ptr->GetFullFlag() & UCF_NLSAUTH) == 0)
			{
				packet.status = 5;
				memset(packet.salt, 0, 32);
				memset(packet.pubkey, 0, 32);
